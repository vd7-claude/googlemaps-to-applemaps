...
const GMAPS = 'https://raw.githubusercontent.com/vdutts7/squircle/refs/heads/main/web/google/google-maps.webp';
...
const CopyIco = () => <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"/></svg>;
...
const ChevIco = () => <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="6 9 12 15 18 9"/></svg>;

function useIsMobile() {
  const [m, setM] = useState(() => window.innerWidth < 680);
  useEffect(() => { const q = window.matchMedia('(max-width: 680px)'); const h = e => setM(e.matches); q.addEventListener('change', h); return () => q.removeEventListener('change', h); }, []);
  return m;
}
...
function CopyBtn({ text }) {
  ...
  const go = useCallback(async () => {
    ...
    setOk(true);
    if (r.current) gsap.fromTo(r.current, { scale: 0.88 }, { scale: 1, duration: 0.5, ease: 'elastic.out(1,0.35)' });
    setTimeout(() => setOk(false), 2000);
  }, [text]);
  return <button ref={r} className={`btn-ghost ${ok ? 'copied' : ''}`} onClick={go}>{ok ? <CheckIco /> : <CopyIco />}{ok ? 'Copied' : 'Copy URL'}</button>;
}
...
function MobileDrawer({ parsed, onClose }) {
...
function Result({ parsed, appleUrl }) {
  ...
  // Animate result panel in on mount
  useEffect(() => {
    if (!ref.current) return;
    gsap.fromTo(ref.current, { y: 24, scale: 0.98 }, { y: 0, scale: 1, duration: 0.5, ease: 'power3.out' });
  }, []);
  ...
  return (
    <>
        ...
        <div className="result-grid">
          <div className="result-left">
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <span className="tag tag-src"><img src={GMAPS} width={14} height={14} alt="" style={{ borderRadius: 3 }} /> Google Maps</span>
              <span className="tag tag-type">{TYPES[parsed.type] || parsed.type}</span>
            </div>
            <div className="url-display">{parsed.originalUrl}</div>
            {hasInfo && (mobile
              ? <button className="btn-ghost" onClick={() => { haptic([5]); setDrawer(true); }} style={{ alignSelf: 'flex-start' }}>Details <ChevIco /></button>
              : <Details parsed={parsed} />
            )}
          </div>
          ...
          <div className="result-right">
            {parsed.type === 'short' ? (
              ...
              <>
                <span className="tag tag-dest"><img src={AMAPS} width={14} height={14} alt="" style={{ borderRadius: 3 }} /> Apple Maps</span>
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  <a href={appleUrl} target="_blank" rel="noopener noreferrer" className="btn-apple" onClick={() => haptic([15, 50, 10])}>
                    <img src={AMAPS} width={18} height={18} alt="" style={{ borderRadius: 5 }} />
                    Open in Apple Maps
                  </a>
      ...
      {drawer && <MobileDrawer parsed={parsed} onClose={() => setDrawer(false)} />}
    </>
  );
}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    gsap.fromTo(previewRef.current, { y: 32, scale: 0.97 }, { y: 0, scale: 1, duration: 0.6, ease: 'power3.out' });
  }, [result]);
  ...
  const convert = useCallback((urlOverride) => {
    ...
    if (parsed.error && parsed.type !== 'short') { haptic([30, 50, 30]); setError(parsed.error); return; }
  ...
  const loadExample = url => { haptic([5]); setInput(url); setResult(null); setError(null); requestAnimationFrame(() => convert(url)); };
    ...
    <div className="app">
      ...
      {result?.parsed?.coords && (
        <div className="preview-section" ref={previewRef}>
          {result.parsed.name && (<><PlaceCard name={result.parsed.name} coords={result.parsed.coords} /><div className="preview-divider" /></>)}
          <MapPreview coords={result.parsed.coords} zoom={Math.min(result.parsed.zoom || 14, 17)} name={result.parsed.name} />
        </div>
      )}

      <div className="footer">
        <span>All URL formats</span><span>·</span><span>Runs in browser</span><span>·</span><span>No data sent</span>
      </div>
    </div>
  );
}
